...
import { SimpleLocationService, type LocationSearchParams } from '$lib/simpleLocationService';
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    if (batch && Array.isArray(batch)) {
      try {
        const results = await SimpleLocationService.batchSearchLocations(batch);
        return json({
          success: true,
          results,
          count: results.length,
          successful: results.filter(r => r !== null).length,
          source: 'nominatim-only'
        });
      } catch (error) {
    ...
    const result = await SimpleLocationService.searchLocation(searchParams);
    ...
    console.error('Simple location search API error:', error);
    return json(
      { 
